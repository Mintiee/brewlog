...
import { openDatabase, idbRequest } from "./idb";
...
/**
 * One transaction against the shared connection.
 *
 * This used to open the database and close it again per operation, so draining N
 * queued writes cost 2 + 2N open/close cycles — all of it on the path where the device
 * has just come back online and is trying to flush work. lib/store/idb.ts memoises the
 * connection instead.
 */
function tx<T>(mode: IDBTransactionMode, run: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> {
  return openDatabase(DB_NAME, DB_VERSION, (db) => {
    if (!db.objectStoreNames.contains(STORE)) db.createObjectStore(STORE, { keyPath: "id" });
  }).then((db) => idbRequest<T>(db, STORE, mode, run));
}
